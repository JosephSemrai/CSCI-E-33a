...
def socketConnection(data):
    data.channels = {};
    sockets[data.id] = data;
    print(f"Added socket {data.id}")
    print(f"Added socket {data.id}")
    print(f"Added socket {data.id}")
    print(f"Added socket {data.id}")
    print(f"Added socket {data.id}")
...
def rtcJoin(data):
    ...
    for id in channels[channel]:
        channels[channel][id].emit('addPeer', {'peer_id': request.sid, 'should_create_offer': False})
    channels[channel][request.sid] = request
    request["channels"][channel] = channel;
    print(f"[{id}] join {data}")
...
def run():
    ...
    print("MAIN EXECUTED")
    db.create_all()
    socketio.run(app,ssl_context=('cert.pem', 'cert.key'))
